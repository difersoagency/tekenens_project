<form action="{{ route('partner.update',$data->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <div class="mb-3    @if($data->photo == '') d-none @endif" id="get">
            <div class="col-sm-3">
            </div>
            <div class="col-sm-9">
                <img id="get_photo"
                    alt="get image" style="max-height: 250px; max-width: 300px" src="{{ asset('storage/'.$data->photo) }}">
                    <button class="btn btn-danger" id="reset_upload" type="button" onclick="delete_image()"><i class="fa fa-trash-o"></i></button>
            </div>
        </div>

        <div class="col-sm-9 @if($data->photo != '') d-none @endif" id="upload">
